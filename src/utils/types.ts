...
export interface Bot extends TypedEmitter<BotEvents> {
  ...
  findPlayer: (filter: any) => Entity | Entity[];
  findPlayers: (filter: any) => Entity | Entity[];
  uuidToUsername: {};
  _playerFromUUID: (uuid: any) => Player;
  bot: any;
  vehicle: any;
  targetDigFace: any;
  lastDigTime: any;
  _updateBlockState(position: any, arg1: number): any;
  signBook: (slot: any, pages: any, author: any, title: any) => Promise<void>;
  username: string;
  ...
  dig: ((
    ...
    ((
      block: Block,
      forceLook: boolean | "ignore",
      digFace: "auto" | Vec3 | "raycast"
    ) => Promise<void>);

  stopDigging: (err?) => void;
...
export interface GameState {
  serverBrand: any;
  enableRespawnScreen: any;
  serverViewDistance: any;
  levelType: LevelType;
  gameMode: GameMode;
  hardcore: boolean;
  dimension: Dimension;
...
export interface creativeMethods {
  ...
  clearInventory: () => Promise<void> | Promise<void[]>;
...
export interface Chest extends Window<StorageEvents> {
  constructor();
...
export interface Furnace extends Window<FurnaceEvents> {
  fuel: number;
  progress: number;
...
export interface Dispenser extends Window<StorageEvents> {
  constructor();
...
export interface EnchantmentTable extends Window<ConditionalStorageEvents> {
...
export interface Villager extends Window<ConditionalStorageEvents> {
