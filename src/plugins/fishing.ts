...
export default (bot: Bot) => {
  ...
  let fishingTask: any = createDoneTask();
  let lastBobber = null;
  ...
  bot._client.on("world_particles", (packet) => {
    ...
    if (
      packet.particleId === (parts?.fishing ?? parts.bubble).id &&
      packet.particles === 6 &&
      pos.distanceTo(new Vec3(packet.x, pos.y, packet.z)) <= 1.23
    ) {
      bot.activateItem();
  ...
  async function fish() {
    if (!fishingTask.done) {
      fishingTask.cancel(
        new Error("Fishing cancelled due to calling bot.fish() again")
      );
    }
