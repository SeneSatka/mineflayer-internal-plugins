...
export default (bot: Bot) => {
  async function openEnchantmentTable(enchantmentTableBlock) {
    ...
    let ready = false;
    const enchantmentTable = (await bot.openBlock(
      enchantmentTableBlock
    )) as any;
    if (!enchantmentTable.type.toString().startsWith("minecraft:enchant")) {
      throw new Error(
        "Expected minecraft:enchant when opening table but got " +
          enchantmentTable.type
      );
    }
    ...
    enchantmentTable.targetItem = function () {
      return this.slots[0];
    };
