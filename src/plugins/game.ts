...
export default (bot: Bot, options: BotOptions) => {
  ...
  function handleRespawnPacketData(packet) {
    bot.game.levelType =
      packet.levelType ?? (packet.isFlat ? "flat" : "default");
    bot.game.hardcore = packet.isHardcore ?? Boolean(packet.gameMode & 0b100);
    bot.game.gameMode = parseGameMode(packet.gameMode) as GameMode;
    ...
    if (bot.supportFeature("dimensionDataInCodec")) {
      ...
      if (packet.worldType) {
        ...
        const { minY, height } =
          bot.registry.dimensionsByName[bot.game.dimension];
        bot.game.minY = minY;
    ...
    } else if (bot.supportFeature("dimensionDataIsAvailable")) {
      // 1.18
    ...
    if (packet.difficulty) {
      bot.game.difficulty = difficultyNames[packet.difficulty] as Difficulty;
    }
  }
