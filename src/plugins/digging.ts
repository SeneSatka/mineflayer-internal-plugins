import { createDoneTask, createTask } from "../utils/promise_utils";
import { iterators } from "prismarine-world";
import { Bot, BotEvents } from "../utils/types";
import { Vec3 } from "vec3";
import { Block } from "prismarine-block";
const BlockFaces = iterators.BlockFace;

export default (bot: Bot) => {
  let swingInterval = null;
  let waitTimeout = null;
  let diggingTask: any = createDoneTask();
  ...
  async function dig(
    ...
    if (forceLook !== "ignore") {
      if (digFace?.x || digFace?.y || digFace?.z) {
        ...
        if (digFace.x) {
          ...
          bot.targetDigFace =
            digFace.y > 0 ? BlockFaces.TOP : BlockFaces.BOTTOM;
        } else if (digFace.z) {
          bot.targetDigFace =
            digFace.z > 0 ? BlockFaces.SOUTH : BlockFaces.NORTH;
        }
        await bot.lookAt(
          block.position
            .offset(0.5, 0.5, 0.5)
            .offset(digFace.x * 0.5, digFace.y * 0.5, digFace.z * 0.5),
          forceLook
        );
        ...
        const dy =
          bot.entity.position.y + bot.entity.height - (block.position.y + 0.5);
        ...
        for (const i in visibleFaces) {
          ...
          const rayBlock = bot.world.raycast(
            startPos,
          ...
          if (rayBlock) {
            if (
              startPos.distanceTo(rayBlock.intersect) <
    ...
    bot.stopDigging = () => {
      ...
      const stoppedBecauseOfNewDigRequest = !currentBlock.position.equals(
        bot.targetDigBlock.position
      );
      const cancellationDiggingFace = !stoppedBecauseOfNewDigRequest
        ? bot.targetDigFace
        : 0;
      //@ts-ignore
  ...
  function canDigBlock(block) {
    return (
      ...
      block.position
        .offset(0.5, 0.5, 0.5)
        .distanceTo(bot.entity.position.offset(0, 1.65, 0)) <= 5.1
    );
  }
