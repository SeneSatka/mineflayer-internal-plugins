...
function calcExposure(playerPos, explosionPos, world) {
  ...
  for (pos.y = playerPos.y; pos.y <= playerPos.y + 1.8; pos.y += 1.8 * dy) {
    for (
      pos.x = playerPos.x - 0.3 + d3;
      pos.x <= playerPos.x + 0.3;
      pos.x += 0.6 * dx
    ) {
      for (
        pos.z = playerPos.z - 0.3 + d4;
        pos.z <= playerPos.z + 0.3;
        pos.z += 0.6 * dz
      ) {
...
function getDamageAfterAbsorb(damages, armorValue, toughness) {
  ...
  const var4 = Math.min(
    Math.max(armorValue - damages / var3, armorValue * 0.2),
    20
  );
...
export default (bot: Bot) => {
  ...
  bot.getExplosionDamages = (
    ...
    let damages = Math.floor(
      (impact * impact + impact) * damageMultiplier * power + 1
    );
    ...
    if (!rawDamages && targetEntity.attributes["generic.armor"]) {
      ...
      const armorToughness = getAttributeValue(
        //@ts-ignore
        targetEntity.attributes[armorThoughnessKey]
      );
