...
export default (bot: Bot) => {
  ...
  let eatingTask = createDoneTask() as any;
  ...
  bot.on("heldItemChanged", (heldItem) => {
    ...
    if (
      heldItem?.type === previousHeldItem?.type &&
      heldItem?.count === previousHeldItem?.count
    ) {
  ...
  async function consume() {
    if (!eatingTask.done) {
      eatingTask.cancel(
        new Error("Consuming cancelled due to calling bot.consume() again")
      );
    }

    if (
      bot.game.gameMode !== "creative" &&
      !ALWAYS_CONSUMABLES.includes(bot.heldItem.name) &&
      bot.food === 20
    ) {
  ...
  async function transfer(options) {
    ...
    async function transferOne() {
      ...
      if (
        ...
        const mcDataEntry = bot.registry.itemsArray.find(
          (x) => x.id === itemType
        );
  ...
  function copyInventory(window) {
    ...
    for (let i = window.inventoryStart; i < window.inventoryEnd; i++) {
      ...
      if (!Item.equal(bot.inventory.slots[slot], item, true))
  ...
  async function waitForWindowUpdate(window, slot) {
    if (window.type === "minecraft:inventory") {
      ...
      if (
        slot <= 1 &&
        !window.selectedTrade.tradeDisabled &&
        expectTradeUpdate(window)
      ) {
  ...
  function confirmTransaction(windowId, actionId, accepted) {
    ...
    if (
      click === undefined ||
      !windowClickQueue.some((clicks) => clicks.id === actionId)
    ) {
    ...
    function onAccepted() {
      ...
      window.acceptClick(click, 0); //@ts-ignore
  ...
  async function clickWindow(slot, mouseButton, mode) {
    ...
    if (slot >= bot.QUICK_BAR_START && bot.lastDigTime != null) {
      ...
      while (
        (timeSinceLastDig = Date.now() - bot.lastDigTime) < DIG_CLICK_TIMEOUT
      ) {
    ...
    } else if (bot.supportFeature("actionIdUsed")) {
      // <= 1.16.5
      bot._client.write("window_click", {
        windowId: window.id,
        slot,
    ...
    if (bot.supportFeature("transactionPacketExists")) {
      ...
      const [success] = await withTimeout(response, WINDOW_TIMEOUT).catch(
        () => {
          throw new Error(
            `Server didn't respond to transaction for clicking on slot ${slot} on window with id ${window?.id}.`
          );
        }
      );
      if (!success) {
