...
export default (bot: Bot) => {
  ...
  async function openVillager(villagerEntity) {
    const villagerType = entitiesByName.villager
      ? entitiesByName.villager.id
      : entitiesByName.Villager.id;
    assert.strictEqual(villagerEntity.entityType, villagerType);
    let ready = false;
    //@ts-ignore
    const villagerPromise = bot.openEntity(villagerEntity) as any;
    bot._client.on(tradeListPacket, gotTrades);
    const villager = await villagerPromise;
    if (
      villager.type !== "minecraft:villager" &&
      villager.type !== "minecraft:merchant"
    ) {
      throw new Error(
        "Expected minecraft:villager or minecraft:mechant type, but got " +
          villager.type
      );
    }
    ...
    async function gotTrades(packet) {
      ...
      villager.trades = packet.trades.map((trade) => {
        trade.inputs = [
          (trade.inputItem1 = Item.fromNotch(
            trade.inputItem1 || { blockId: -1 }
          )),
        ];
        if (trade.inputItem2?.itemCount != null) {
          trade.inputs.push(
            (trade.inputItem2 = Item.fromNotch(
              trade.inputItem2 || { blockId: -1 }
            ))
          );
        }
        ...
        trade.hasItem2 = !!(
          trade.inputItem2 &&
          trade.inputItem2.type &&
          trade.inputItem2.count
        );
        trade.outputs = [
          (trade.outputItem = Item.fromNotch(
            trade.outputItem || { blockId: -1 }
          )),
        ];

        if (trade.demand !== undefined && trade.specialPrice !== undefined) {
          // the price is affected by demand and reputation
          const demandDiff = Math.max(
            0,
            Math.floor(
              trade.inputItem1.count * trade.demand * trade.priceMultiplier
            )
          );
          trade.realPrice = Math.min(
            Math.max(
              trade.inputItem1.count + trade.specialPrice + demandDiff,
              1
            ),
  ...
  async function trade(villager, index, count) {
    ...
    assert.ok(
      Trade.maximumNbTradeUses - Trade.nbTradeUses > 0,
      "trade blocked"
    );
    assert.ok(Trade.maximumNbTradeUses - Trade.nbTradeUses >= count);

    const itemCount1 = villager.count(
      Trade.inputItem1.type,
      Trade.inputItem1.metadata
    );
    const hasEnoughItem1 = itemCount1 >= Trade.realPrice * count;
    let hasEnoughItem2 = true;
    let itemCount2 = 0;
    if (Trade.hasItem2) {
      itemCount2 = villager.count(
        Trade.inputItem2.type,
        Trade.inputItem2.metadata
      );
    ...
    if (bot.supportFeature("selectingTradeMovesItems")) {
      // 1.14+ the server moves items around by itself after selecting a trade
      const proms = [];
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!bot.supportFeature("setSlotAsTransaction")) {
        ...
        if (slot1) {
          ...
          const updatedSlot1 =
            updatedCount1 <= 0 ? null : { ...slot1, count: updatedCount1 };
        ...
        if (slot2) {
          ...
          const updatedSlot2 =
            updatedCount2 <= 0 ? null : { ...slot2, count: updatedCount2 };
