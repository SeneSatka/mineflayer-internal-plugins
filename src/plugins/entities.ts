...
import * as conv from "../utils/conversions";
import { Bot, Player } from "../utils/types";
...
export default (bot: Bot) => {
  ...
  bot._playerFromUUID = (uuid) =>
    Object.values(bot.players).find((player) => player.uuid === uuid);
  ...
  bot._client.on("entity_equipment", (packet) => {
    ...
    if (packet.equipments !== undefined) {
      packet.equipments.forEach((equipment) =>
        entity.setEquipment(
          equipment.slot,
          equipment.item ? Item.fromNotch(equipment.item) : null
        )
      );
    } else {
      entity.setEquipment(
        packet.slot,
        packet.item ? Item.fromNotch(packet.item) : null
      );
    }
  ...
  // What is internalId?
  const entityDataByInternalId = Object.fromEntries(
    bot.registry.entitiesArray.map((e) => [e.internalId, e])
  );
  ...
  function addNewPlayer(entityId, uuid, pos) {
    ...
    if (
      bot.players[entity.username] !== undefined &&
      !bot.players[entity.username].entity
    ) {
  ...
  function handleBotUsedFireworkRocket(fireworkEntityId, fireworkInfo) {
    ...
    let flightDur =
      fireworkInfo?.nbtData?.value?.Fireworks?.value?.Flight.value ?? 1;
    if (typeof flightDur !== "number") {
      flightDur = 1;
    }
    const baseDuration = 10 * (flightDur + 1);
    const randomDuration =
      Math.floor(Math.random() * 6) + Math.floor(Math.random() * 7);
  ...
  const hasFireworkSupport =
    fireworkEntityName !== undefined &&
    fireworkMetadataIdx !== undefined &&
    fireworkMetadataIsOpt !== undefined;
  ...
  bot._client.on("entity_metadata", (packet) => {
    ...
    if (bot.supportFeature("mcDataHasEntityMetadata")) {
      ...
      if (
        hasFireworkSupport &&
        fireworkEntityName === entity.name &&
        metas.attached_to_target !== undefined
      ) {
      ...
      if (metas.shared_flags != null) {
        ...
        } else if (entity.crouching) {
          // prevent the initial entity_metadata packet from firing off an uncrouch event
      ...
      const typeSlot =
        (bot.supportFeature("itemsAreAlsoBlocks") ? 5 : 6) +
        (bot.supportFeature("entityMetadataHasLong") ? 1 : 0);
      const slot = packet.metadata.find((e) => e.type === typeSlot);
      if (
        entity.name &&
        (entity.name.toLowerCase() === "item" ||
          entity.name === "item_stack") &&
        slot
      ) {
        bot.emit("itemDrop", entity);
      }
      ...
      if (hasFireworkSupport && fireworkEntityName === entity.name) {
        const attachedToTarget = packet.metadata.find(
          (e) => e.key === fireworkMetadataIdx
        );
        if (attachedToTarget !== undefined) {
          ...
          if (entityId !== undefined && entityId === bot.entity?.id) {
            const fireworksItem = packet.metadata.find(
              (e) => e.key === fireworkMetadataIdx - 1
            );
  ...
  bot._client.on("player_info", (packet) => {
    ...
    if (bot.supportFeature("playerInfoActionIsBitfield")) {
      for (const item of packet.data) {
        let player: Player = bot.uuidToUsername[item.uuid]
          ? bot.players[bot.uuidToUsername[item.uuid]]
          : null;
        let newPlayer = false;
        ...
        if (packet.action & 1) {
          ...
          obj.displayName =
            player.displayName ||
            //@ts-ignore
            new ChatMessage({ text: "", extra: [{ text: item.player.name }] });
        ...
        if (item.displayName) {
          ...
          //@ts-ignore
          obj.displayName = new ChatMessage({
            text: "",
            extra: [{ text: player.username || obj.username }],
          });
        ...
        const playerEntity = Object.values(bot.entities).find(
          (e) => e.type === "player" && e.username === player.username
        );
      ...
      for (const item of packet.data) {
        let player: any = bot.uuidToUsername[item.UUID]
          ? bot.players[bot.uuidToUsername[item.UUID]]
          : null;
        if (packet.action === 0) {
          let newPlayer = false;
          ...
          if (!player) {
            ...
            player = bot.players[item.name] = {
              ...
              uuid: item.UUID,
              //@ts-ignore
              displayName: new ChatMessage({
                text: "",
                extra: [{ text: item.name }],
              }),
          ...
          const playerEntity = Object.values(bot.entities).find(
            (e) => e.type === "player" && e.username === item.name
          );
          player.entity = playerEntity;
          ...
          if (packet.action === 1) {
            ...
            //@ts-ignore
            player.displayName = new ChatMessage({
              text: "",
              extra: [{ text: player.username }],
            });
          } else if (packet.action === 3 && item.displayName) {
            player.displayName = ChatMessage.fromNotch(item.displayName);
  ...
  bot._client.on("player_remove", (packet) => {
    for (const uuid of packet.players) {
      const player = bot.uuidToUsername[uuid]
        ? bot.players[bot.uuidToUsername[uuid]]
        : null;
...
function extractSkinInformation(properties) {
  ...
  const skinTexture = JSON.parse(
    Buffer.from(props.textures.value, "base64").toString("utf8")
  );

  const skinTextureUrl = skinTexture?.textures?.SKIN?.url ?? undefined;
  const skinTextureModel =
    skinTexture?.textures?.SKIN?.metadata?.model ?? undefined;
