...
export default (bot: Bot) => {
  function setCommandBlock(pos, command, options: any = {}) {
    assert.strictEqual(
      bot.player.gamemode,
      1,
      new Error(
        "The bot has to be in creative mode to open the command block window"
      )
    );
    assert.notStrictEqual(pos, null);
    assert.notStrictEqual(command, null);
    assert.strictEqual(
      bot.blockAt(pos).name.includes("command_block"),
      true,
      new Error("The block isn't a command block")
    );
    ...
    if (bot.supportFeature("usesAdvCmd") || bot.supportFeature("usesAdvCdm")) {
      const pluginChannelName = bot.supportFeature("usesAdvCdm")
        ? "MC|AdvCdm"
        : "MC|AdvCmd";
