...
export default (bot, { physicsEnabled, maxCatchupTicks }) => {
  ...
  function tickPhysics(now) {
    ...
    if (bot.physicsEnabled && shouldUsePhysics) {
      physics
        .simulatePlayer(new PlayerState(bot, controlState), world)
        .apply(bot);
  ...
  bot._client.on("position", (packet) => {
    ...
    const newYaw =
      (packet.flags & 8 ? conv.toNotchianYaw(bot.entity.yaw) : 0) + packet.yaw;
    const newPitch =
