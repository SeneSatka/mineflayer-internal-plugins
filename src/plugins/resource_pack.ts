...
export default (bot: Bot) => {
  ...
  bot._client.on("add_resource_pack", (data) => {
    // Emits the same as resource_pack_send but sends uuid rather than hash because that's how active packs are tracked
  ...
  bot._client.on("remove_resource_pack", (data) => {
    // Doesn't emit  anything because it is removing rather than adding
    ...
    if (data.uuid === undefined) {
      ...
      try {
        ...
        console.error(
          "Tried to remove UUID but it was not in the active list."
        );
      }
    }
  });
