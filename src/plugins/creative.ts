...
import { once, onceWithCleanup, sleep } from "../utils/promise_utils";
...
export default (bot: Bot) => {
  ...
  async function setInventorySlot(slot, item) {
    ...
    if (creativeSlotsUpdates[slot]) {
      throw new Error(
        `Setting slot ${slot} cancelled due to calling bot.creative.setInventorySlot(${slot}, ...) again`
      );
    }
    ...
    await onceWithCleanup(bot.inventory, `updateSlot:${slot}`, {
      checkCondition: (oldItem, newItem) =>
        item === null
          ? newItem === null
  ...
  async function clearInventory(): Promise<void[]> {
    return Promise.all(
      bot.inventory.slots
        .filter((item) => item)
        .map((item) => setInventorySlot(item.slot, null))
    );
  }
