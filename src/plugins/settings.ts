...
export default (bot, options) => {
  function setSettings(settings) {
    ...
    if (typeof bot.settings.viewDistance === "string") {
      ...
      typeof bot.settings.viewDistance === "number" &&
      bot.settings.viewDistance > 0
    ) {
      // Make sure view distance is a valid # || should be 2 or more
      viewDistanceBits = bot.settings.viewDistance;
    }
    assert.ok(
      viewDistanceBits != null,
      `invalid view distance setting: ${bot.settings.viewDistance}`
    );
    ...
    const skinParts =
      (bot.settings.skinParts.showCape << 0) |
