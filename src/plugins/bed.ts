...
export default (bot: Bot) => {
  ...
  function isABed(block: Block) {
    return beds.has(block.name);
  }
  function parseBedMetadata(bedBlock: Block) {
    const metadata: {
      part: boolean;
    ...
    if (bot.supportFeature("blockStateId")) {
      const state =
        bedBlock.stateId -
        bot.registry.blocksByStateId[bedBlock.stateId].minStateId;
  ...
  async function sleep(bedBlock: Block) {
    ...
    if (
      !thunderstorm &&
      !(bot.time.timeOfDay >= 12541 && bot.time.timeOfDay <= 23458)
    ) {
      ...
      if (!metadata.part) {
        // Is foot
        const upperBlock = bot.blockAt(
          bedBlock.position.plus(metadata.headOffset)
        );

        if (isABed(upperBlock)) {
          headPoint = upperBlock.position;
        } else {
          const lowerBlock = bot.blockAt(
            bedBlock.position.plus(metadata.headOffset.scaled(-1))
          );
      ...
      const oppositeCardinal =
        (metadata.facing + 2) % CARDINAL_DIRECTIONS.length;
      ...
      if (
        botPos.x > seClickCorner.x ||
        botPos.x < nwClickCorner.x ||
        botPos.y > seClickCorner.y ||
        botPos.y < nwClickCorner.y ||
        botPos.z > seClickCorner.z ||
        botPos.z < nwClickCorner.z
      ) {
      ...
      if (
        ...
        bot.supportFeature("creativeSleepNearMobs")
      ) {
        // If in creative mode the bot should be able to sleep even if there are monster nearby (starting in 1.13)
        const nwMonsterCorner = headPoint.offset(
          monsterRange[1],
          -6,
          monsterRange[2]
        ); // North-West lower corner
        const seMonsterCorner = headPoint.offset(
          monsterRange[3],
          4,
          monsterRange[0]
        ); // South-East upper corner
        ...
        for (const key of Object.keys(bot.entities)) {
          ...
          if (entity.kind === "Hostile mobs") {
            ...
            if (
              entityPos.x <= seMonsterCorner.x &&
              entityPos.x >= nwMonsterCorner.x &&
              entityPos.y <= seMonsterCorner.y &&
              entityPos.y >= nwMonsterCorner.y &&
              entityPos.z <= seMonsterCorner.z &&
              entityPos.z >= nwMonsterCorner.z
            ) {
